"""
Full end-to-end example using ANY custom LLM.

- You only need to implement a minimal adapter that returns model output as text.

Use this when:
- your LLM is internal
- your LLM is behind a REST API
- your LLM is experimental
- your LLM is not OpenAI / Anthropic / Ollama / from HuggingFace

Demonstrates ALL core features:
- failure detection
- root cause analysis
- recommendations
- causal graph learning
- causal graph visualization
- causal explanation
"""

import sys
from pathlib import Path

# Add parent directory to path so we can import llm_failure_debugger
sys.path.insert(0, str(Path(__file__).parent.parent))

from llm_failure_debugger import (
    Debugger, CustomAdapter, ModelCapabilities, ModelInternals
)

class AnyLLMAdapter(CustomAdapter):
    """
    Adapter for any arbitrary / custom LLM.

    The user is only required to implement `generate()`
    to return a string output from their model.

    All failure analysis, causal learning, and explanations
    are handled by the library.
    """

    def __init__(self, **config):
        self.config = config
        super().__init__()

    def _declare_capabilities(self) -> ModelCapabilities:
        """
        Declare what this LLM exposes.
        By default, assume a pure text-generation model.
        """
        return ModelCapabilities(
            supports_logits=False,
            supports_embeddings=False,
            supports_nli=False,
            supports_rag=False,
            supports_tools=False,
            supports_multi_output=False,
        )

    def generate(self, prompt: str, **kwargs) -> str:
        """
        🚨 USER IMPLEMENTATION REQUIRED. 🚨
        
        Generate text from the LLM.

        🔧 Replace the body of this method with your actual
        model call (REST API, SDK, local inference, etc.).

        Must return the model output as a string.
        """
        # example: return your_model.generate(prompt, **kwargs)

        raise NotImplementedError(
            "Implement your LLM call here and return generated text."
        )

    def extract_internals(self, prompt, output, **kwargs) -> ModelInternals:
        """
        🚨 Optional 🚨

        Return model internals if available.

        Most custom / closed models will return empty internals.
        This is fully supported.
        """
        return ModelInternals()


# --------------------------------------------------
# Create debugger
# --------------------------------------------------

adapter = AnyLLMAdapter()
debugger = Debugger(adapter)


# --------------------------------------------------
# Test prompts
# --------------------------------------------------

prompts = [
    "Who was the first human to land on Mars?",
    "Explain why India won the 2026 FIFA World Cup.",
    "List all Nobel Prize winners in Physics from 2020 to 2025.",
    "In 2024 Apple released an iPhone with a nuclear battery. Explain.",
]


# --------------------------------------------------
# Run prompts
# --------------------------------------------------

for i, prompt in enumerate(prompts, 1):
    print("\n" + "=" * 80)
    print(f"PROMPT {i}")
    print("=" * 80)
    print(prompt)

    result = debugger(prompt)

    print("\nMODEL OUTPUT")
    print("-" * 80)
    print(result.output_text)

    if not result.has_failures:
        print("\n✅ No failures detected.")
        continue

    print("\n🚨 FAILURES DETECTED")
    for failure in result.failures:
        print(f"\nFailure Type : {failure.failure_type.value}")
        print(f"Severity     : {failure.severity.value}")
        print(f"Confidence   : {failure.confidence:.2f}")

        if failure.signals:
            print("Signals:")
            for s in failure.signals:
                print(f"  - {s}")

        if failure.root_causes:
            print("Root Causes:")
            for rc in failure.root_causes:
                print(f"  - {rc}")

        if failure.recommendations:
            print("Recommendations:")
            for rec in failure.recommendations:
                print(f"  - {rec}")

        # ------------------------------
        # Explicit failure localization
        # ------------------------------
        if failure.localization:
            print("Localization:")
            print(f"  Level      : {failure.localization.level}")
            print(f"  Evidence   : {failure.localization.evidence}")
            print(f"  Confidence : {failure.localization.confidence:.2f}")

    # ------------------------------
    # Prompt repair (automatic)
    # ------------------------------
    if result.repaired_prompt:
        print("\n🔧 REPAIRED PROMPT")
        print(result.repaired_prompt)


# --------------------------------------------------
# Learned failure prediction (from causal graph)
# --------------------------------------------------

print("\n" + "=" * 80)
print("FAILURE PREDICTION (LEARNED)")
print("=" * 80)

predictions = debugger.predict_failures()

if not predictions:
    print("No learned failure patterns yet.")
else:
    for p in predictions:
        print(
            f"Likely failure: {p.failure_type} | "
            f"Probability: {p.probability:.2f} | "
            f"Based on: {p.based_on}"
        )


# --------------------------------------------------
# Causal graph visualization
# --------------------------------------------------

print("\n" + "=" * 80)
print("CAUSAL GRAPH")
print("=" * 80)

graph_path = debugger.visualize_causal_graph(show=True)

if graph_path:
    print(f"📈 Causal graph saved to: {graph_path}")


# --------------------------------------------------
# Causal explanation
# --------------------------------------------------

print("\nCAUSAL EXPLANATION")
debugger.explain_causal_graph()
